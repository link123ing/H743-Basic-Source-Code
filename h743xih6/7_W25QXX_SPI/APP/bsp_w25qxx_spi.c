...
void W25QXX_init(void)
{
    ...
    Uart_Printf(&huart1,"FLASH ID:%04x\r\n", g_norflash_type);
}
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    NORFLASH_CS(0);
    spi_read_write_byte(command);  /* 发送写寄存器命令 */
...
uint16_t norflash_read_id(void)
{
    ...
    spi_read_write_byte(0);
    deviceid = spi_read_write_byte(0xFF) << 8;     /* 读取1字节高8位bit */
    deviceid |= spi_read_write_byte(0xFF);         /* 读取1字节低8位bit */
    NORFLASH_CS(1);
