...
/******************************************************************************************/
/* OV2640 设备地址定义 */
#define OV2640_ADDR         0X60    /* OV2640的SCCB设备地址 (写操作), 读操作时为0X61 */

/* OV2640 寄存器定义 */
#define OV2640_PID           0X2642  /* OV2640芯片ID */
...
/* OV2640 复位/电源引脚定义 (根据实际硬件修改) */
#define OV2640_RST_GPIO_PORT    GPIOG
#define OV2640_RST_GPIO_PIN     GPIO_PIN_12
#define OV2640_RST_GPIO_CLK_ENABLE()  do{ __HAL_RCC_GPIOG_CLK_ENABLE(); }while(0)

#define OV2640_PWDN_GPIO_PORT   GPIOD
#define OV2640_PWDN_GPIO_PIN    GPIO_PIN_13
#define OV2640_PWDN_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOD_CLK_ENABLE(); }while(0)
...
/******************************************************************************************/
/* 函数声明 */
uint8_t ov2640_read_reg(uint16_t reg);                  /* 读寄存器 */
uint8_t ov2640_write_reg(uint16_t reg, uint8_t data);   /* 写寄存器 */
void ov2640_pwdn_set(uint8_t sta);                      /* 电源控制 */
uint8_t ov2640_init(void);                              /* 初始化OV2640 */
void ov2640_rgb565_mode(void);                          /* 切换为RGB565输出模式 */
void ov2640_jpeg_mode(void);                            /* 切换为JPEG输出模式 */
