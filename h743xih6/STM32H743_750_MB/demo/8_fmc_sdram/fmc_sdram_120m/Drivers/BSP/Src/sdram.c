...
void SDRAM_Init(void)
{
  ...
  /* 我们使用的SDRAM刷新周期为64ms,SDCLK=240/2=120Mhz,行数为4096(2^12) */
  /* 所以,COUNT=64*1000*120/4096-20=1855 */    
...
{  
	...
	for(i=0;i<64*1024*1024;i+=16*1024)      /* 每隔16K字节,写入一个数据,总共写入4096个数据,刚好是64M字节 */
	{
		*(__IO uint32_t*)(FMC_SDRAM_ADDR+i)=temp; 
		temp++;
	}	
    
 	for(i=0;i<64*1024*1024;i+=16*1024)      /* 依次读出之前写入的数据,进行校验 */
	{	
